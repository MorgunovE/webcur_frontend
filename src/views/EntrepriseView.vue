  ...
  <v-app>
    ...
    <v-main>
      <v-container>
        ...
        <v-row>
          <v-col cols="12" md="6">
            <v-card v-if="entreprise">
              <v-card-title>
                <img :src="entreprise.image" alt="Logo" height="40" v-if="entreprise.image" />
              ...
              <v-card-text>
                ...
                <div><strong>Site web:</strong> <a :href="entreprise.website" target="_blank">{{ entreprise.website }}</a></div>
        ...
        <v-row>
          <v-col v-for="pop in entreprisesPopulaires" :key="pop.symbole" cols="12" md="4">
            <v-card>
              <v-card-title>
              ...
              <v-card-actions>
                <v-btn :to="`/companies/${pop.symbole}`" color="primary" text>Voir</v-btn>
              </v-card-actions>
            </v-card>
          </v-col>
        </v-row>
...
async function chargerDonnees() {
  ...
  await store.dispatch('entreprises/chargerHistoriqueEntreprise', { symbole: symbole.value, jours: 30 });
